...
# crontab to automate the exc file
crontab -l  : list all the cron exc condition in the user
crontab -e  : edit the crontab file
service cron start and service cron stop

# add the lib for run the exc
1. put the .so file in the /usr/local/lib and change the name lib(name)
2. put the .h file in the /usr/local/include
3. add the /usr/local/lib to the /etc/ld.so.conf
4. ldconfig to make it effect
while linking the lib use -l(name)

# seee the process

1. ps to look the process and pid
    ps命令查找与进程相关的PID号：
